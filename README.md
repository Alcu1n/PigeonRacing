...
- 仅给已有会员补充登录凭据，不创建新会员，也不改变会员启用/禁用状态。
- 目标会员已有手机号、手机号被其他会员占用、棚号不存在或文件内棚号/手机号重复时，整行跳过。
- 手机号必须为 11 位中国大陆手机号；密码去除首尾空格后必须为 6–128 个字符。
- 预览和错误报告不显示密码明文；确认导入时会重新校验数据库状态。
- 密码以哈希保存，导入成功后会员首次登录必须修改密码。
...
生产脚本调用 `deploy-member-assets-to-oss.sh` 时会按锁文件安装依赖，默认执行 `npx vite build`，不会额外跑 `vue-tsc` 类型检查。正式发布前仍建议在本地或 CI 跑完整前端测试和构建。
