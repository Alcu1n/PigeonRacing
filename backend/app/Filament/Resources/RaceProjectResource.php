<?php
// [IN]: RaceProject model records / 赛事项目模型记录
// [OUT]: Filament project rule configuration screens / Filament 项目规则配置页面
...
class RaceProjectResource extends Resource
{
    ...
    public static function form(Schema $schema): Schema
    {
        return $schema->components([
            ...
            TextInput::make('group_size')->label('每注所需羽数')->helperText('1 显示在单羽矩阵；大于 1 显示为多羽组合。')->numeric()->minValue(1)->required(),
            TextInput::make('price_cent')->label('每注金额（分）')->numeric()->minValue(0)->required(),
            TextInput::make('sort_order')->label('排序')->numeric()->default(0),
            Toggle::make('is_enabled')->label('启用')->default(true),
            Toggle::make('allow_repeat_pigeon_in_project')->label('允许同足环在本项目重复使用')->default(false),
            TextInput::make('max_entries_per_member')->label('每会员最大报名注数')->numeric(),
