...
@php
    $currency = $record->currency_code ?? $record->race?->currency_code;
    $money = fn (int $cent): string => \App\Support\CurrencyFormatter::format($cent, $currency);
@endphp
<x-filament-panels::page>
    <div class="space-y-6">
        <x-filament::section :heading="__('编辑说明')">
            <div class="grid gap-3 text-sm text-gray-700 dark:text-gray-200 md:grid-cols-3">
                <div>{{ __('赛事') }}：<strong>{{ $record->race?->name }}</strong></div>
                <div>{{ __('棚号') }}：<strong>{{ $record->member?->loft_number }}</strong></div>
                <div>{{ __('参赛名') }}：<strong>{{ $record->member?->participant_name }}</strong></div>
            </div>
            <div class="mt-3 text-xs leading-5 text-gray-500 dark:text-gray-400">
                {{ __('保存后数据默认已确认；递进阶段会按阶段顺序重新校验，无法匹配上一阶段确认组的后续数据会被自动移除。') }}
            </div>
        </x-filament::section>

        @if ($singleProjects !== [])
            <x-filament::section :heading="__('单羽组')">
                <div class="overflow-x-auto rounded-lg border border-gray-200 dark:border-gray-700">
                    <table class="w-full min-w-[760px] text-sm">
                        <thead class="bg-gray-50 dark:bg-gray-800">
                            <tr>
                                <th class="sticky left-0 z-10 w-52 bg-gray-50 px-3 py-2 text-left dark:bg-gray-800">{{ __('足环号码') }}</th>
        ...
        @if ($multiProjects !== [])
            <x-filament::section :heading="__('多羽组')">
                <div class="space-y-5">
                        ...
                        <div class="rounded-lg border border-gray-200 p-4 dark:border-gray-700">
                            <div class="mb-3 flex flex-wrap items-center justify-between gap-2">
                                <div>
                                    ...
                                    <div class="text-xs text-gray-500">{{ __('每组 :count 羽，:amount/组', ['count' => $project['group_size'], 'amount' => $money((int) $project['price_cent'])]) }}</div>
                                </div>
                            ...
                            <div class="space-y-3">
                                ...
                                @empty
                                    <div class="rounded-md bg-gray-50 px-3 py-2 text-sm text-gray-500 dark:bg-gray-800">{{ __('暂无报名组。') }}</div>
                                @endforelse
                            </div>
                        </div>
                    @endforeach
        ...
        @if ($progressiveCategories !== [])
            <x-filament::section :heading="__('递进阶段项目')">
                <div class="space-y-6">
                            ...
                            <div class="space-y-5">
                                @foreach ($category['stages'] as $stage)
                                    <div class="rounded-lg bg-gray-50 p-4 dark:bg-gray-800/70">
                                        <div class="mb-3 flex flex-wrap items-center justify-between gap-2">
                                            <div>
                                                <div class="font-medium text-gray-950 dark:text-white">{{ __('第 :order 阶段：:name', ['order' => $stage['stage_order'], 'name' => $stage['name']]) }}</div>
                                                <div class="text-xs text-gray-500">{{ __('每组 :count 羽，:amount/组', ['count' => $stage['group_size'], 'amount' => $money((int) $stage['price_cent'])]) }}</div>
                                            </div>
                                        ...
                                        <div class="space-y-3">
                                            ...
                                            @empty
                                                <div class="rounded-md bg-white px-3 py-2 text-sm text-gray-500 dark:bg-gray-900">{{ __('暂无阶段数据。') }}</div>
                                            @endforelse
                                        </div>
                                    </div>
                                @endforeach
