    ...
    <div class="space-y-6">
        <x-filament::section :heading="__('选择会员')">
            ...
            <div class="mt-3 text-xs leading-5 text-gray-500 dark:text-gray-400">
                {{ __('本页用于直接维护某个会员在「:name」下的全部阶段数据。第一阶段仍作为资格基准，不计入报名金额。', ['name' => $record->name]) }}
            </div>
        </x-filament::section>

        @if ($memberId)
            <x-filament::section :heading="__('阶段数据')">
                @if ($pigeons === [])
                    <div class="rounded-lg bg-warning-50 px-4 py-3 text-sm text-warning-700 dark:bg-warning-500/10 dark:text-warning-300">
                        {{ __('当前会员名下没有正常状态足环，请先到足环管理中维护足环。') }}
                    </div>
                @endif
                    ...
                    @foreach (($categoryData['stages'] ?? []) as $stage)
                        <div class="rounded-lg border border-gray-200 p-4 dark:border-gray-700">
                            ...
                            <div class="space-y-3">
                                ...
                                @empty
                                    <div class="rounded-md bg-gray-50 px-3 py-2 text-sm text-gray-500 dark:bg-gray-800">{{ __('暂无阶段数据。') }}</div>
                                @endforelse
                            </div>
                        </div>
                    @endforeach
